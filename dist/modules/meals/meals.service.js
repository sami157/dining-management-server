...
const editMealRegistration = async (registrationId, numberOfMeals, currentUser) => {
    ...
    if (currentUser.role !== 'admin') {
        ...
        const deadline = calculateMealDeadline(new Date(registration.date), registration.mealType, mealConfig.customDeadline || null, mealDeadlineConfig);
...
const bulkRegisterMealsForUser = async (month, userId) => {
    ...
    const registeredSet = new Set(existingRegistrations.map(reg => `${reg.date.toISOString()}_${reg.mealType}`));
    const toInsert = [];
    for (const schedule of schedules) {
